...
## Current State
- **Version:** 1.108.25 — secret-file detection hardening. `is_secret_file()` now matches basenames plus a whole-segment `secret`/`secrets` directory rule (gated to non-source data/credential extensions), replacing the old full-path `*secret*` substring match that silently dropped legit subtrees like `services/secrets-manager/`. The directory rule preserves detection of real credentials stored as data files in conventional secret stores (k8s/Terraform/Docker, e.g. `config/secrets/database.yaml`) that a naive basename-only fix would have started indexing. Closes the report behind PR #317 (in-house clean-room fix; contributor PR closed with thanks). Shipped via GitHub-release wheel (PyPI account suspension under appeal). 5 new regression tests in `test_security.py`. **Post-release docs fix (unreleased — rides the next release):** added quarantine-aware install notes to README's "Start fast" and "Works with" sections and switched the Codex recommended-install line to `git+https` (was bare `pip install -U jcodemunch-mcp`). Root cause of the #308 `-32000` reports: only the README hero block was repointed at the GitHub-release wheel during quarantine; the body sections (Start fast, MCP-client wiring, groq extras, Codex venv) still instructed the bare PyPI name, so anyone following them past the top configured a command uvx can't resolve. Notes are version-agnostic (no wheel-pin to bump); revert all three when PyPI clears.
